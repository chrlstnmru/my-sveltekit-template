 ...
 * ```
 */
export function shortcut(node: HTMLElement, options: Options[] | Options) {
  ...
  $effect(() => {
    ...
    const removeListeners = optionsArr.map((opt) =>
      on(node, opt.event ?? 'keydown', (e) => handleKeydown(e, opt))
    );
